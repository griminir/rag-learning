...
export async function updateFileCache(filePath: string, chunkCount: number): Promise<void> {
  const currentHash = await hashFile(filePath);
  const cache = await loadCache();
...
export async function getCachedChunkCount(filePath: string): Promise<number | null> {
  const cache = await loadCache();
