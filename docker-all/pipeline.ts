...
export async function pipeline(inputPath: string | string[]) {
  ...
  for (const filePath of filePaths) {
    ...
    if (!fileChanged) {
      ...
      console.log(
        `⚡ Fast path: File unchanged, ${cachedCount} chunks already in database`
      );
      totalSkipped++;
    ...
    const { needsEmbedding, existing } = await checkExistingChunks(
      collection,
      chunks,
      filePath
    );
    ...
    console.log(
      `✓ File complete: ${ids.length} chunks (${newEmbeddings.length} new, ${existing.length} reused)`
    );
  }
  ...
  console.log(
    `   Files processed: ${filePaths.length - totalSkipped}/${filePaths.length}`
  );
