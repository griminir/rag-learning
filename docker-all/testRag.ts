...
async function main() {
  const { query, minScore, k, verbose, stream, fallback } = parseArgs(
  ...
  if (!query) {
    ...
    console.log(
      '  tsx testRag.ts "Explain embeddings" --k=3 --verbose --stream'
    );
    process.exit(1);
  }
  ...
  if (fallback)
    console.log(
      'Mode: Fallback enabled (will use general knowledge if needed)'
    );
  ...
  try {
    ...
    response.sources.forEach((src, idx) => {
      console.log(
        `  ${idx + 1}. ${src.source} (relevance: ${(src.score * 100).toFixed(
          1
        )}%)`
      );
    });
    ...
    if (
      ...
      console.error(
        '  Ensure the ollama container is running: docker compose up -d'
      );
      console.error(
        '  And the model is pulled: docker exec rag-ollama ollama pull llama3.1:8b'
      );
    } else if (
      error.message?.includes('ECONNREFUSED') &&
