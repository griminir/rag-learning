...
export async function generateAnswer(
  ...
  const systemPrompt = allowFallback ? SYSTEM_PROMPT_FALLBACK : SYSTEM_PROMPT_STRICT;
 ...
 */
export function formatContext(documents: Array<{ text: string; metadata: any; score: number }>): string {
  ...
  return documents
    .map((doc, idx) => {
      ...
      return `[Document ${idx + 1}] (Source: ${source}, Relevance: ${(doc.score * 100).toFixed(1)}%)
${doc.text}`;
    })
    .join("\n\n---\n\n");
}
