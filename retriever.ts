...
export async function createRetriever(collection: Collection, options?: RetrieverOptions) {
  const opts = Object.assign({ k: 5, modelName: "Xenova/all-MiniLM-L6-v2", minScore: 0 }, options || {});

  // Lazy-load the embedding extractor once and reuse
  console.log("Loading embedding model for retriever...");
  const extractor = await transformersPipeline("feature-extraction", opts.modelName);
  ...
  async function retrieve(query: string, k?: number, overrideMinScore?: number): Promise<RetrievedDocument[]> {
    const n = k || opts.k || 5;
    const effectiveMinScore = typeof overrideMinScore === "number" ? overrideMinScore : (opts.minScore ?? 0);
    ...
    returnedIds.forEach((docId: string, idx: number) => {
      ...
      const cosineSimilarity = typeof rawDistance === "number" ? 1 - rawDistance : 0;
      const normalizedScore = Math.max(0, Math.min(1, (cosineSimilarity + 1) / 2));
